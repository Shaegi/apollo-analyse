...
export const getAverageExecutionTimeInMs = (info: TracingInfo) =>
  convertNSToMs(info.tracingInfos.reduce((acc, curr) => acc + curr.duration, 0) / info.tracingInfos.length)
...
export const deepMap = (
  obj: Record<string, any> & { children?: any[] },
  map: (node: typeof obj) => Record<string, any>
) => {
  if (obj.children) {
    return {
      ...map(obj),
